...
class DEC(torch.nn.Module):
    ...
    def forward(self, received):
        ...
        copy_out1[:, 0:5, :] = rnn_out1[:, 0:1, :]
        copy_out2[:,0:5,:] = rnn_out2[:,0:1,:]
        for t in range(5,self.args.block_len):
            a1 = self.attention(rnn_out1[:,t:t+1,:],rnn_out1[:,t-5:t,:],5)
            c1 = torch.bmm(a1,rnn_out1[:,t-5:t,:])
            new_out1 = self.context(torch.cat((c1,rnn_out1[:,t:t+1,:]),dim=2))
            copy_out1[:,t:t+1,:]= new_out1
            a2 = self.attention(rnn_out2[:, t:t + 1, :], rnn_out2[:, t-5:t, :], 5)
            c2 = torch.bmm(a2,rnn_out2[:,t-5:t,:])
